function initAnexos(){
	window.parent.$("#ecm-navigation-inputFile-clone").change(function(e){
        var fileName = "anexo___" + e.target.files[0].name;
...
function showCameraCustom(id, idInputAnexo, isPaiFilho) {
	...
	if (!getMobile()) {
		...
		if (tabAttachments) {
			...
			console.log("###_01 parameter: " + parameter);
			//$("#" + parameter).val(parameter);
		}
	} else {
		JSInterface.showCamera(parameter);
		console.log("###_02 parameter: " + parameter);
		$("#" + parameter).val(parameter);
	}
}
...
function controlaAnexo(termoBotaoAnexo) {
	...
	var canShowAnexo = (atividade == INICIO_0 || atividade == INICIO) && (mode != 'VIEW');
	var canShowLimpar = (atividade == INICIO_0 || atividade == INICIO) && (mode != 'VIEW');
